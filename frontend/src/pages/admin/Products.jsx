          ...
          <tbody className="divide-y divide-[color:var(--brand-border)]">
            {items.map((p) => (
              <tr key={p.id} data-testid={`admin-product-${p.slug}`}>
                ...
                <td className="p-3 text-right sku">₹{p.price}</td>
                <td className="p-3 text-right sku">{p.stock}</td>
                <td className="p-3 text-center">{p.moq}</td>
                <td className="p-3 text-right"><button onClick={() => del(p.id)} className="text-red-500" data-testid={`del-${p.id}`}><Trash size={14} /></button></td>
              </tr>
            ))}
          </tbody>
        </table>
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <label className="text-xs"><span className="text-slate-500">Brand</span>
                <select value={f.brandId} onChange={(e) => setF({ ...f, brandId: e.target.value })} className="mt-1 w-full border rounded px-3 py-2" data-testid="prod-brand">
